...
func (toa *TraefikOidcAuth) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if err == nil && session != nil {
		...
		log(toa.Config.LogLevel, LogLevelInfo, "Token verification failed: %s", err.Error()) // More descriptive message
	}
...
func (toa *TraefikOidcAuth) handleUnauthorized(rw http.ResponseWriter, req *http.Request) {
	// This function is no longer used since we always return 401 directly in ServeHTTP
	http.Error(rw, http.StatusText(http.StatusUnauthorized), http.StatusUnauthorized)
}
